...
func newHarness(t *testing.T) *harness {
	...
	grace := &web.User{Ref: graceRef, ID: "0199bf3c-7a1e-7c2b-9f10-000000000002", Username: "grace", Name: "Grace"}
	...
	content := &fakeContent{posts: map[string]*web.Post{
		draftID: {Ref: draftRef, ID: draftID, AuthorRef: adaRef, Title: "A draft", Body: "Hidden.", Status: web.StatusDraft},
	}}
...
func (h *harness) request(t *testing.T, method, target, token string, form url.Values) *httptest.ResponseRecorder {
	t.Helper()
...
func TestDraftLooksMissingToEveryoneWithoutAccess(t *testing.T) {
	...
	assert.Equal(t, http.StatusNotFound, h.request(t, http.MethodGet, "/posts/"+draftID, "", nil).Code)
	assert.Equal(t, http.StatusNotFound, h.request(t, http.MethodGet, "/posts/"+draftID, graceName, nil).Code)
...
func TestEditingSomeoneElsesPostIsRefused(t *testing.T) {
	...
	assert.Equal(t, http.StatusForbidden, h.request(t, http.MethodGet, "/admin/posts/"+draftID+"/edit", graceName, nil).Code)

	h.authz.allow(adaRef, web.ActionPostUpdate, draftRef)
	assert.Equal(t, http.StatusOK, h.request(t, http.MethodGet, "/admin/posts/"+draftID+"/edit", adaToken, nil).Code)
}
...
func TestCreatingAPostGrantsItsAuthorOwnership(t *testing.T) {
	...
	assert.ElementsMatch(t,
		[]string{web.ActionPostRead, web.ActionPostUpdate, web.ActionPostDelete, web.ActionPostPublish},
		grant.Actions,
	)
}
...
func TestDeletingAPostRemovesItsGrants(t *testing.T) {
	...
	response := h.request(t, http.MethodPost, "/admin/posts/"+draftID+"/delete", adaToken, url.Values{})
...
func TestMalformedIdentifiersAreNotFound(t *testing.T) {
	...
	assert.Equal(t, http.StatusNotFound, h.request(t, http.MethodGet, "/posts/not%20an%20id", "", nil).Code)
	assert.Equal(t, http.StatusNotFound, h.request(t, http.MethodGet, "/admin/users/not%20an%20id", adaToken, nil).Code)
}
...
func TestDeletingAUserRemovesTheirRolesAndGrants(t *testing.T) {
	...
	response := h.request(t, http.MethodPost, "/admin/users/0199bf3c-7a1e-7c2b-9f10-000000000002/delete", adaToken, url.Values{})

	require.Equal(t, http.StatusSeeOther, response.Code)
	assert.Equal(t, []string{graceRef}, h.auth.deleted)
	assert.Equal(t, []string{graceRef}, h.authz.purged, "roles and grants must not outlive the account")
}
...
func TestDeletingYourOwnAccountIsRefused(t *testing.T) {
	...
	response := h.request(t, http.MethodPost, "/admin/users/"+adaID+"/delete", adaToken, url.Values{})
